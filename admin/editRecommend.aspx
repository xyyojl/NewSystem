<%@ Page Title="" Language="C#" MasterPageFile="~/admin/MasterPage.master" AutoEventWireup="true" CodeFile="editRecommend.aspx.cs" Inherits="admin_editRecommend" %>
...
<asp:Content ID="Content2" runat="server" 
   ...
   <form id="form1" runat="server">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        ...
        SelectCommand="SELECT * FROM [recommend]" 
        ConflictDetection="CompareAllValues" 
        DeleteCommand="DELETE FROM [recommend] WHERE [ID] = @original_ID AND (([Title] = @original_Title) OR ([Title] IS NULL AND @original_Title IS NULL)) AND (([Author] = @original_Author) OR ([Author] IS NULL AND @original_Author IS NULL)) AND (([Time] = @original_Time) OR ([Time] IS NULL AND @original_Time IS NULL)) AND (([Content] = @original_Content) OR ([Content] IS NULL AND @original_Content IS NULL)) AND (([PageCount] = @original_PageCount) OR ([PageCount] IS NULL AND @original_PageCount IS NULL))" 
        InsertCommand="INSERT INTO [recommend] ([Title], [Author], [Time], [Content], [PageCount]) VALUES (@Title, @Author, @Time, @Content, @PageCount)" 
        OldValuesParameterFormatString="original_{0}" 
        UpdateCommand="UPDATE [recommend] SET [Title] = @Title, [Author] = @Author, [Time] = @Time, [Content] = @Content ,[PageCount] = @PageCount WHERE [ID] = @original_ID">
        <%--UpdateCommand="UPDATE [recommend] SET [Title] = @Title, [Author] = @Author, [Time] = @Time, [Content] = @Content, [PageCount] = @PageCount WHERE [ID] = @original_ID AND (([Title] = @original_Title) OR ([Title] IS NULL AND @original_Title IS NULL)) AND (([Author] = @original_Author) OR ([Author] IS NULL AND @original_Author IS NULL)) AND (([Time] = @original_Time) OR ([Time] IS NULL AND @original_Time IS NULL)) AND (([Content] = @original_Content) OR ([Content] IS NULL AND @original_Content IS NULL)) AND (([PageCount] = @original_PageCount) OR ([PageCount] IS NULL AND @original_PageCount IS NULL))">--%>
